...
#region Inits
	...
	void InitConfigs() {
		...
		InitViews(NotifyViewType.Error);
		_currentVersion       = PlayerPrefs.GetInt("Version", -1);
	}

	void InitViews(NotifyViewType viewType) {
		switch (viewType) {
	...
	private void InitUserProperty() {
		Application.RequestAdvertisingIdentifierAsync(
		                                              (string advertisingId, bool trackingEnabled, string error) => {
			                                              ...
			                                              FrameView.Url = _serverConfig.url;
			                                              PlayBrowserGame();
		                                              }
		                                             );
	}
...
#region LoadScene
	...
	private void LoadWebView() => SceneManager.LoadSceneAsync(scene.frameScene);
...
#region Offline&Browser
	...
	private void PlayOfflineGame() {
		if (_serverConfig.updateEnable && (_currentVersion != _serverConfig.version || _serverConfig.forceUpdate)) {
			InitViews(NotifyViewType.Changelog);
		}
		else {
...
#region Connection
	...
	private void Connect() {
		...
		if (Application.internetReachability != NetworkReachability.NotReachable) {
			...
			if (test.EnableServerConfigTest)
				...
				StartCoroutine(LighteggUtil.GetDataFromBmob<ServerConfig>(configUrl[0], appId, restApiKey, OnDataReceived,
				                                                     () => { view.errorView.Show(); }));
		}
		else {
			view.errorView.Show();
	...
	void OnDataReceived(ServerConfig serverConfig) {
		...
		FragmentView.Url = _serverConfig.privacyUrl?.Trim();
		InitProfile();
...
#region Data
	...
	private class ViewConfig {
		public NotifyView  changelogView, errorView;
	}

	[System.Serializable]
	private class SceneConfig {
		public string frameScene = "Frame", builtInScene = "Main";
	}
